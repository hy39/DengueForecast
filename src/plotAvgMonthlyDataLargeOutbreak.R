...
mintemp <-  as.numeric(c(mintemp, Temp.Kao[i,2:(2+nrow(Temp.Kao)-1)]))
...
wea <- data.frame(year,month,maxrain,mintemp,cond)


ggplot(wea, aes(x=month, y=mintemp, color=cond)) + 
  geom_point() +
  stat_summary(fun.y = mean, geom = "line", size=0.7) +
  ylab(expression("Monthly minimum temperature " ( degree*C))) +
...
ggplot(wea, aes(x=month, y=maxrain, color=interaction(cond), shape=cond)) + 
  geom_point() +
  stat_summary(fun.y = mean, geom = "line", size=0.7) +
  ylab(expression("Monthly maximum 24-hour rainfall (mm)")) +
  scale_x_continuous(breaks=1:12,labels=month.abb) +
  scale_colour_discrete("cond") +
  scale_shape_manual("cond",values=c(16,17))
