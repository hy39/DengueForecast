...
Rain.Kao <- read.csv("../資料/高雄雨量(2).csv", header=T) # Both average and maximum rainfall
...
Kao <- cbind(Case.Kao[-13,], Temp.Kao[-13,2:9], Rain.Kao[-13,2:9], Rain.Kao[-13,14:21]) 
Kao2017 <- cbind(1, Temp.Kao[13,2:9], Rain.Kao[13,2:9], Rain.Kao[13,14:21]) 

#Kao <- cbind(Case.Kao[c(-13,-12,-11),], Temp.Kao[c(-13,-12,-11),2:9], Rain.Kao[c(-13,-12,-11),2:9], Rain.Kao[c(-13,-12,-11),14:21]) 
#Kao2016 <- cbind(1, Temp.Kao[12,2:9], Rain.Kao[12,2:9], Rain.Kao[12,14:21]) 
n <- dim(Kao)[1]
k <- dim(Kao)[2] - 3
...
glm_fin <- stepAICc(glm(CASE3 ~ 1, family = poisson, data = Kao),
                ...
                #scope = CASE3 ~ T1 + T2 + T3 + T4 + T5 + T6 + T7 + T8 + R1 + R2 + R3 + R4 + R5 + R6 + R7 + R8 + AR1 + AR2 + AR3, 
                direction = "both")
...
#pred2016 <- exp(sum(fit.glm$coeff*cbind(1,Kao2016[par])))
##真實發病數與估計發病數的比較
y_actual <- Kao$CASE3
